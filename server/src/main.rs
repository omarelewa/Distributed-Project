...
fn main() {
    ...
    let t1 = receiver.listen(); // listen for requests and send acknowledgements to sender

    let t2 = receiver.log_stats(); // log stats every 10 seconds to stats.txt file

    receiver.handle_requests(); // handle requests and send responses

    t1.join().unwrap(); // join listen thread to main thread to keep program running

    t2.join().unwrap(); // join log_stats thread to main thread to keep program running
}
